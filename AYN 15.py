...
def covid():
    ...
    city_dict = {} #defining the dict that will contain the list of all the countries
    for x in range(len(json["Countries"])):
        city_dict.update({json["Countries"][x]["Country"]: x})

    if country not in city_dict and country != "": #if you search without typing a country in the serach box you get global news
    ...
    covid_news_label["text"] = covid_text #covid text goes to covid_news_label's "text" parameter
