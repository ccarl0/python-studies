...
def covid():
    ...
    covid_text = f"Today's new cases: {json['Global']['NewConfirmed']}" \
    ...
    covid_news_label["text"] = covid_text
    print(covid_text)
...
covid_news_button = tk.Button(frame, text="Get Covid-19 News!", command = covid, width = 19)
covid_news_button.pack(anchor = "w", side = "bottom")

covid_country_text = tk.StringVar()
covid_country_entry = tk.Entry(frame, textvariable = covid_country_text, width = 23)
